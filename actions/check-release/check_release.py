...
def validate(version, ref, default_branch, sha, checkout_sha, compiler_pin, allow_default=False, allow_nightly_bootstrap=False):
    ...
    if compiler is None and not bootstrap:
    ...
    return {"version": version, "sha": sha, "compiler-pin": compiler_pin, "maintenance-branch": line,
            "compiler-channel": "nightly-bootstrap" if bootstrap else "stable"}


def verify_compiler_release(compiler_pin, release, channel="stable"):
    if channel not in {"stable", "nightly-bootstrap"}:
...
def main():
    ...
    result = validate(os.environ["RELEASE_VERSION"], os.environ["GITHUB_REF"],
                      ...
                      checkout_sha, Path(".roc-version").read_text().strip(), allowed == "true", bootstrap == "true")
