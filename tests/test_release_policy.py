...
class ReleasePolicyTests(unittest.TestCase):
    def check(self, version="0.1.1", ref="refs/heads/release/0.1.x", **kwargs):
        return policy.validate(version, ref, "main", "a" * 40, kwargs.pop("checkout_sha", "a" * 40), **kwargs)

    def test_patch_and_prerelease(self):
        for version in ("0.1.0", "0.1.1", "0.1.12-rc.1", "0.1.0-rc1"):
            self.assertEqual(self.check(version)["release-line"], "release/0.1.x")
    ...
    def test_wrong_line_tags_and_pr_refs_rejected(self):
        for ref in ("refs/heads/release/0.2.x", "refs/tags/0.1.1", "refs/pull/1/merge", "refs/heads/feature"):
    ...
    def test_entrypoint_writes_only_validated_outputs(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            env = {"GITHUB_EVENT_NAME": "workflow_dispatch", "GITHUB_REF": "refs/heads/release/0.1.x",
